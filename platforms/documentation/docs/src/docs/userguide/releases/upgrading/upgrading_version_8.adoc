...
=== Defining secondary variants for primary variants does not remove primary variants

It is possible to define [Secondary Variants](https://docs.gradle.org/8.9/userguide/publishing_customization.html#sec:publishing-custom-components) for consumable "primary" variants that lack artifacts.

Defining secondary variants for a primary variant lacking artifacts used to result in the primary variant being removed from consideration during variant selection.
Now such primary variants remain selectable, even if they lack artifacts.

In very rare cases, when a variant request's attributes are underspecified, this change may cause a different variant to be selected than in previous versions of Gradle.
